    ...
    if($action == 'clientIncome') {
        ...
        while($row = mysqli_fetch_array($query)) {
            ...
            if($ctr % 2 == 0) {
                ...
                $pdf->Cell(40, 8, 'Php ' . number_format($credit, 2, '.', ','), 1, 0, 'C', true);
                $pdf->Cell(40, 8, 'Php ' . number_format($debit, 2, '.', ','), 1, 0, 'C', true);
                $pdf->Cell(40, 8, $scanIncome, 1, 0, 'C', true);
                $pdf->Ln();
            } else {
                $pdf->Cell(76, 8, $name, 1, 0, 'C');
                $pdf->Cell(40, 8, 'Php ' . number_format($credit, 2, '.', ','), 1, 0, 'C');
                $pdf->Cell(40, 8, 'Php ' . number_format($debit, 2, '.', ','), 1, 0, 'C');
        ...
        $pdf->Cell(98, 8, 'Php ' . number_format($credit, 2, '.', ','), 1, 0, 'C');
        $pdf->Cell(98, 8, 'Php ' . number_format($debit, 2, '.', ','), 1, 0, 'C');
